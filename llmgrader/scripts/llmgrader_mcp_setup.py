...
TEMPLATE_PACKAGE = "llmgrader.mcp.resources"
TEMPLATE_NAME = "mcp.json"
PYTHON_PLACEHOLDER = "__LLMGRADER_PYTHON__"
IMPORT_PROBE = "import llmgrader.mcp.server"
...
def render_mcp_config(*, python_path: str) -> str:
    ...
    config["servers"]["llmgrader"]["command"] = python_path
...
def validate_python_interpreter(python_path: str) -> None:
    ...
    details = stderr or stdout or "Unknown error while importing llmgrader.mcp.server"
    raise RuntimeError(
        "The selected Python interpreter cannot import llmgrader.mcp.server: "
        f"{python_path}\n{details}"
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a VS Code MCP configuration for llmgrader in a workspace."
    )
    parser.add_argument(
        "--workspace",
        default=".",
