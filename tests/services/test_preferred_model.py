...
def test_the_symbolic_names_are_the_tier_names() -> None:
    assert resolve_preferred_model("cheap").id == DEFAULT_MODEL
    assert resolve_preferred_model("strong").id == DEFAULT_PROJECT_MODEL
...
def test_preferred_model_for_reads_the_question_dict() -> None:
    assert preferred_model_for({"preferred_model": "strong"}) == DEFAULT_PROJECT_MODEL
...
UNIT_XML = """<unit id="preferred_model_unit" title="Preferred Model Unit" version="1.0">
  <question qtag="q_strong" preferred_model="strong">
...
def test_the_fixture_unit_loads(grader: Grader) -> None:
    assert set(_questions(grader)) == {"q_strong", "q_terra", "q_none", "q_typo"}


def test_a_symbolic_preference_resolves_per_question(grader: Grader) -> None:
    questions = _questions(grader)

    assert preferred_model_for(questions["q_strong"], "q_strong") == DEFAULT_PROJECT_MODEL
...
def test_preferred_model_is_used_when_the_client_sends_none(client, monkeypatch) -> None:
    assert _model_used(client, monkeypatch, "q_strong") == DEFAULT_PROJECT_MODEL
...
def test_an_explicit_client_model_wins(client, monkeypatch) -> None:
    ...
    used = _model_used(client, monkeypatch, "q_strong", model=DEFAULT_MODEL)

    assert used == DEFAULT_MODEL


def test_no_preference_falls_back_to_the_default(client, monkeypatch) -> None:
    assert _model_used(client, monkeypatch, "q_none") == DEFAULT_MODEL


def test_an_unresolvable_preference_falls_back_and_does_not_raise(client, monkeypatch) -> None:
    assert _model_used(client, monkeypatch, "q_typo") == DEFAULT_MODEL
...
def test_the_unit_payload_carries_raw_and_resolved_preferences(client) -> None:
    ...
    assert items["q_strong"]["preferred_model"] == "strong"
    assert items["q_strong"]["preferred_model_resolved"] == DEFAULT_PROJECT_MODEL
...
def test_the_unit_payload_still_hides_the_solution(client) -> None:
    ...
    assert "solution" not in items["q_strong"]
    assert "grading_notes" not in items["q_strong"]
